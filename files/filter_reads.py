...
def filter_reads(input_bam, output_bam, min_overhang, min_underhang, non_ag_mm_threshold):
    """
    # Step 3: filter reads
    ...
    for read in i:
        ...
        if(softclip):
            if (len(softclip) == 2):
                ...
                read_seq = read_seq[:-(int(softclip2))] # remove softclipped reads
            softclip1 = softclip[0]
            read_seq = read_seq[int(softclip1):] # remove softclipped reads
                
        """
        # 5b) Check for small junction overhangs. If junction over hang is small, remove read
        """
        ...
        if(overhangs):
            ...
            if(len(overhangs) == 1):
                if(int(overhangs[0]) < min_overhang):
                    # print("overhang 1 does not meet the min overhang reqs",overhangs[0])
                    # print(read)
                    # flag = 3
                    continue
            elif(len(overhangs) == 2):
                if ((int(overhangs[1]) < min_overhang) or (int(overhangs[0]) < min_overhang)):
                    # print("overhang 2 does not meet the min overhang reqs",overhangs[1])
                    # print(read)
                    # flag = 4
                    continue
        ...
        """
        # Manually setting reversed reads to 'sense' strand per truseq library protocols
        """
        ...
        if(mismatches):
            ...
            for mismatch in mismatches:
                ...
                read_pos = read_pos + int(mismatch[0])
                ...
                if(not((ref_allele == 'A' and read_allele == 'G' and sense == True) or \
                    ...
                    non_ag_mm_counts = non_ag_mm_counts + 1
...
''' % (program_shortdesc, str(__date__))
    ...
    try:
        ...
        parser.add_argument("-i", "--input", dest="input_bam", help="sorted rmdup bam file for filtering", required = True)
        parser.add_argument("-o", "--output", dest="output_bam", help="output bam filtered per editing rules", required = True)
        parser.add_argument("-j", "--junction_overhang", dest="min_overhang", help="minimum junction overhang needed to start counting mutations", required = False, type=int, default = 10 )
        parser.add_argument("-e", "--edge_mutation", dest="min_underhang", help="minimum nt away from read end to call valid mutation", required = False, type=int, default = 0 )
        parser.add_argument("-ag", "--non_ag_threshold", dest="non_ag_mm_threshold", help="allow up to [n] non-AG mutations in a read before tossing", required = False, type=int, default = 1 )
        # Process arguments
        ...
        filter_reads(input_bam, output_bam, min_overhang, min_underhang, non_ag_mm_threshold)
        return 0
